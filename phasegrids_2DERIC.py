# Code to generate the grids of phasemaps at different times for the 2D ERIC model.
...
def phase_map_2DERIC(N, T, dt, K, L, omega_initial, phi_initial):
...
def generate_plot_data_2DERIC(args):
    N, T, dt, K, L, omega_initial, phi_initial = args
    return phase_map_2DERIC(N, T, dt, K, L, omega_initial, phi_initial)
...
a_values = np.linspace(0.8, 1.2, 10)  # range of a in K = a \Delta_{\omega}
L_values  = np.linspace(1.6, 2.0, 10) # range of \Lambda values

# uncomment below for the 5x5 grids

#a_values = np.linspace(0.8, 1.2, 5)  # range of a in K = a \Delta_{\omega}
#L_values  = np.linspace(1.6, 2.0, 5) # range of \Lambda values
...
for T in T_values:
    ...
    params = [(N, T, dt, a * (w_max - w_min) , L, omega_initial, phi_initial) for a in a_values for L in L_values]
    # Use multiprocessing to generate plot data
    
    with Pool() as p:   
        plot_data = p.map(generate_plot_data_2DERIC, params)
