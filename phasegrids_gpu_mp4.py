# Code for generating .mp4 movies showing phase evolution for the different phase oscillator models described in the paper: "Foci, waves, excitability: self-organization of phase waves in a model of asymmetrically coupled embryonic oscillators". Each frame is a 10x10 grid showing the phasemap in the (K,\Lambda) parameter space at a certain timepoint. The parameter ranges can be changed by the user depending on what they want to study. The code is highly modular and additional phase models can be easily incorporated.
#
#
#
# Author: Kaushik Roy
...
class ConfigManager:
    ...
    @staticmethod
    def save_default_config(config_path: str = 'simulation_config_gpu.json'):
    ...
    def create_animation(self, phase_condition: str, model_name: str = 'ERIC_2D', 
        ...
        if save_gif:
            ...
            filename = os.path.join(output_dir, 
                                   f'phasegrids_{phase_condition}_{model_name}_{L_suffix}_jax_gpu_generic.mp4')  
...
Examples:
  ...
  python phasegrids_gpu_mp4.py --generate-config
  
  # Run specific condition and model
  python phasegrids_gpu_mp4.py --config simulation_config_gpu.json --condition randomp --model ERIC_2D
  
  # Run all conditions for Kuramoto model
  python phasegrids_gpu_mp4.py --config simulation_config_gpu.json --model kuramoto_2D --all-conditions
  
  # List available models and conditions
  python phasegrids_gpu_mp4.py --config simulation_config_gpu.json --list-models --list-conditions
        """
    )
    
    parser.add_argument('--config', type=str, default='simulation_config_gpu.json',
                       help='Path to configuration file (default: simulation_config_gpu.json)')
